...
/**
 * This is {@link CasPuzzleCaptchaWebflowConfigurer}.
 *
 * @author Misagh Moayyed
 * @since 5.0.0
 */
@Slf4j
public class CasPuzzleCaptchaWebflowConfigurer extends AbstractCasWebflowConfigurer {

    protected CasCaptchaProperties casCaptchaProperties;

    public CasPuzzleCaptchaWebflowConfigurer(final FlowBuilderServices flowBuilderServices,
                                             final FlowDefinitionRegistry loginFlowDefinitionRegistry,
                                             final ApplicationContext applicationContext,
                                             final CasCaptchaProperties casCaptchaProperties) {
        super(flowBuilderServices, loginFlowDefinitionRegistry, applicationContext, casCaptchaProperties.getCasProperties());
        this.casCaptchaProperties = casCaptchaProperties;
    }
    ...
    private void createValidateRecaptchaAction(final Flow flow) {
        ...
        state.getActionList().add(createEvaluateAction("puzzleCaptchaAction"));
    ...
    private void createInitialRecaptchaEnabledAction(final Flow flow) {
        flow.getStartActionList().add(new Action() {
            ...
            public Event execute(final RequestContext requestContext) {
                WebUtils.putRecaptchaInvisibleIntoFlowScope(requestContext, casCaptchaProperties.getCaptcha().isInvisible());
